...
const FacilityServicesSection = () => {
  const services = [
    ...
    {
      title: "Marble Polishing & Floor Care",
    ...
    },
    {
      ...
      description: "Comprehensive sanitation of every corner, ensuring a sterile and sparkling workspace.",
      process: ["High-touch point disinfection", "Air quality treatment", "Furniture detailing"],
      image: "/images/sanitization-service.jpg",
      icon: (
        <svg className="w-6 h-6 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-b from-[#3d1a0a] via-[#4d2a0f] to-orange-50 py-20 px-6 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto text-center mb-20 relative z-10">
        <p className="text-orange-400 font-medium tracking-[0.3em] uppercase text-xs mb-3">
          Facility Services
        </p>
      ...
      <div className="max-w-5xl mx-auto space-y-12 relative z-10">
        {services.map((service, index) => (
          <div 
            key={index}
            className="group flex flex-col md:flex-row items-stretch bg-white/20 backdrop-blur-lg border border-white/30 rounded-[2.5rem] overflow-hidden shadow-2xl hover:shadow-orange-400/10 transition-all duration-500 hover:-translate-y-2"
          >
                ...
                <ul className="grid grid-cols-1 gap-2">
                  {service.process.map((step, i) => (
                    <li key={i} className="flex items-center text-black text-sm">
                      <span className="w-1.5 h-1.5 rounded-full bg-orange-400 mr-3 shadow-[0_0_8px_rgba(251,146,60,0.8)]" />
                      {step}
                    </li>
                  ))}
                </ul>
...
export default FacilityServicesSection;